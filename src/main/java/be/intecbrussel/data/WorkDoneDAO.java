...
public class WorkDoneDAO {
    ...
    public List<WorkDone> getWorkDoneById(int employeeId, int projectId) throws SQLException {
        Connection connection = ConnectionPort.getConnection();
        Statement statement = connection.createStatement();
        ResultSet rs = statement.executeQuery("SELECT * FROM WorkDone WHERE EmployeeId = ? OR ProjectId = ?");
